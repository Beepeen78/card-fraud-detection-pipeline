...
# Builds features on FULL DF (then splits), restores original row order (prevents label/pred misalignment),
# supports distance bucket + category windows, and chooses an expected feature set safely.
...
LABEL = "is_fraud"
# Use card number as the per-user key for rolling features
ID_COL = "cc_num"
...
def build_features(df: pd.DataFrame, want_cols: set) -> pd.DataFrame:
    """
    Compute only the features the model expects (subset-safe) and
    RESTORE ORIGINAL ROW ORDER at the end to keep labels aligned.
    """
    d = df.copy()
    ...
    # parse time and sort for window calcs
    ...
    # --- per-user rolling features (compute on sorted d) ---
    if ID_COL in d.columns and TIME_COL in d.columns:
        ...
        if "amt" in d.columns:
            ...
            if "mean_amt" in want_cols:
                d["mean_amt"] = g_amt.transform(
                    lambda x: x.rolling(50, min_periods=1).mean()
                )
            if "std_amt" in want_cols:
                d["std_amt"] = g_amt.transform(
                    lambda x: x.rolling(50, min_periods=2).std().fillna(0)
                )
            if "median_amt" in want_cols:
                d["median_amt"] = g_amt.transform(
                    lambda x: x.rolling(50, min_periods=1).median()
                )
            if "max_amt" in want_cols:
                d["max_amt"] = g_amt.transform(
                    lambda x: x.rolling(50, min_periods=1).max()
                )
        ...
        if "amt" in d.columns:
            ...
            if "category" in d.columns:
                need_1h_c = {
                    "txn_count_last_1h_category",
                    "total_amt_last_1h_category",
                } & want_cols
                need_24h_c = {
                    "txn_count_last_24h_category",
                    "total_amt_last_24h_category",
                } & want_cols
                if need_1h_c:
    ...
    # restore ORIGINAL row order so X aligns with y and masks
    d = d.sort_values("__orig_idx__").drop(columns="__orig_idx__").reset_index(
        drop=True
    )
...
def get_expected_features(artifacts_dir: Path):
    """
    Prefer feature_columns.pkl; if it disagrees with model, fall back to model's
    feature list to avoid LightGBM shape errors.
    """
    # 1) Try feature_columns.pkl
    ...
    if cols_from_pkl is not None and cols_from_model:
        ...
        # lengths match → stick with pkl (stable)
...
print(
    f"At policy thresholds -> Precision={prec:.3f} | Recall={rec:.3f} | Flags={(y_pred_pos.mean()*100):.2f}%"
)
...
# Drop the last point (no threshold)
thr = np.asarray(thr)
